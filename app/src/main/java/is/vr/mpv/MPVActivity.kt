...
import com.xreal.glasses.api.Control.INRControlCallback
import com.xreal.glasses.api.Startup
import `is`.xyz.mpv.MPVLib.MpvEvent
import `is`.vr.mpv.databinding.PlayerBinding
...
class MPVActivity : AppCompatActivity(), MPVLib.EventObserver, TouchGesturesObserver {
    ...
    private final var vr = Mpv360Controller();
    ...
    private fun initListeners() {
        with (binding) {
            ...
            vrToggleBtn.setOnClickListener       { vr.toggle() }
            vrProjectionBtn.setOnClickListener   { vr.cycleProjection() }
            vrEyeBtn.setOnClickListener          { vr.switchEye() }
            vrResetButton.setOnClickListener     { vr.resetView() }
            vrMouseButton.setOnClickListener     { vr.toggleMouseLook() }
            vrLogButton.setOnClickListener       { vr.toggleShowValues() }
            vrFovIncreaseBtn.setOnClickListener  { vr.fovIncrease() }
            vrFovDecreaseBtn.setOnClickListener  { vr.fovDecrease() }
            vrFishIncreaseBtn.setOnClickListener { vr.fisheyeFovIncrease() }
            vrFishDecreaseBtn.setOnClickListener { vr.fisheyeFovDecrease() }

            vrXrealButton.setOnClickListener { toggleXrealControl() }
        }
            ...
            if (lockedUI) false
                ...
                when (event.actionMasked) {
                    MotionEvent.ACTION_DOWN -> {
                        tracker.zeroView()
                        vr.onMouseStart(event.x, event.y)
                    }

                    MotionEvent.ACTION_UP -> {
                        vr.onMousePosition(event.x, event.y)
                    }
    ...
    private fun initXrealLibrary() {
        Control.setINRControlCallback(object : INRControlCallback {
            override fun onClientCountChanged(i: Int, i2: Int) {
                //Log.d("TEST", "onClientCountChanged ${i} ${i2}")
            }

            override fun onGlassesAction(i: Int, i2: Int, i3: Int, j: Long) {
                //Log.d("TEST", "onGlassesAction ${i} ${i2} ${i3} ${j}")
            ...
            override fun onGlassesEvent(i: Int, i2: Int, i3: Int, i4: Int, i5: Int, bArr: ByteArray?) {
                //Log.d("TEST", "onGlassesEvent ${i} ${i2} ${i3} ${i4} ${i5} ${bArr}")
            }
        })
        ...
        Control.setImuCallback { t1, _, gx, gy, gz, az, ay, ax ->
            ...
            if (tracker.calibrate(imuSample)) {
                ...
                if (currentTime - lastUpdateTime >= THROTTLE_MS) {
                    val orientation = tracker.getRelativeOrientation()
                    vr.onXrealInput(orientation.yaw, orientation.pitch, orientation.roll)
        ...
        Startup.nativeImuInit()
        Startup.nativeImuPause()
    }

    private fun destroyXrealLibrary() {
        try {
            Startup.nativeImuPause()
            Startup.nativeImuDeInit()
            Startup.nativeGlassesPause()
            Startup.nativeGlassesDeInit()
            Startup.nativeStopService()
            //Startup.nativeStopDeviceLog()
        } catch (e: Exception) {
            Log.d("Exception", e.toString())
        } finally {
            xrealInitiated = false
        }
    }

    private fun toggleXrealControl() {
        if (xrealControlRunning) {
            stopXrealControl()
        } else {
            startXrealControl()
        }
    ...
    override fun onCreate(icicle: Bundle?) {
        ...
        // Parse the intent
        var filepath = parsePathFromIntent(intent)
        ...
        val httpAuth = sharedPrefs.getString("http_auth", "")

        if (httpAuth?.isNotEmpty() == true) {
            filepath = filepath.replace("https://", "https://$httpAuth@");
            filepath = filepath.replace("http://", "http://$httpAuth@");
        }
    ...
    companion object {
        ...
        private const val RESULT_INTENT = "is.is.mpv.vr.mpv.MPVActivity.result"
